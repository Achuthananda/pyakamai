...
def createCPCode(contractId,groupId,name,akhttp,accountSwitchKey=None):
    try:
        params = {}
        ...
        if status == 201:
            ...
            print_log('Successfully created the CP Code {} for {}'.format(cpCode,name))
            return cpCode
        else:
            print_log('Failed to create the CP Code for {} and status code is {}.'.format(name,status))
            return 0
    except Exception as e:
        print('{}:Error Creating the CP Code for {}'.format(e,name),file=sys.stderr)
        print_log('{}:Error Creating the CP Code for {}'.format(e,name))
        exit(3)
